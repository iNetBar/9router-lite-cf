...
export function createDb(env) {
  ...
  // ==================== 应用日志 ====================
  async function addLog(level, msg, data = null) {
    try {
      await run(
        'INSERT INTO app_logs (level, msg, data, created_at) VALUES (?, ?, ?, ?)',
        [level, String(msg).slice(0, 500), data ? String(data).slice(0, 500) : null, new Date().toISOString()]
      );
    } catch (_) {}
  }

  async function getLogs({ level, limit = 200 } = {}) {
    const LOG_LEVELS = { debug: 0, info: 1, warn: 2, error: 3 };
    let sql = 'SELECT level, msg, data, created_at FROM app_logs';
    const params = [];
    if (level && level !== 'all' && LOG_LEVELS[level] !== undefined) {
      const minLevel = LOG_LEVELS[level];
      const levels = Object.entries(LOG_LEVELS).filter(([, v]) => v >= minLevel).map(([k]) => k);
      sql += ` WHERE level IN (${levels.map(() => '?').join(',')})`;
      params.push(...levels);
    }
    sql += ' ORDER BY id DESC LIMIT ?';
    params.push(Math.min(parseInt(limit) || 200, 500));
    const { rows } = await execute(sql, params);
    return { logs: rows.map(r => ({ time: r.created_at, level: r.level, msg: r.msg, data: r.data })), total: rows.length };
  }
