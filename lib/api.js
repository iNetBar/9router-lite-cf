...
function unauthorized() {
  return Response.json({ success: false, error: '未授权：请先登录管理后台', code: 'UNAUTHORIZED' }, { status: 401 });
}
...
export async function handleApi(request, env, ctx) {
  ...
  // 系统设置
  if (path === '/api/settings' && method === 'GET') return Response.json({ fallback: { enabled: true, maxRetries: 3, timeout: 30000 }, rateLimit: { enabled: true, points: 100, duration: 60 }, logging: { enabled: true, level: 'info' } });
  ...
  // 统计
  if (path === '/api/stats') return Response.json(await db.getUsageStats());
  if (path === '/api/stats/detailed') return Response.json(await db.getDetailedStats());

  // 日志（Workers 无持久内存，返回空列表）
  if (path === '/api/logs' && method === 'GET') return Response.json({ success: true, logs: [], total: 0 });
